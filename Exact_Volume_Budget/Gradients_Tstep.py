...
def x_grad_u(RomsNC, var, dx, Masks) :
    """
    compute x-gradient on u points 
    """
    #gradient in x direction [u points]
    dvar_u = np.diff(var, n = 1, axis = 2)*Masks['U_Mask']
...
def x_grad_rho(RomsNC, var, dx, Masks) :
    """
    ...
    dvar_rho = GridShift.Upt_to_Rho(dvar_u)*Masks['RhoMask']
...
def y_grad_rho(RomsNC, var, dy, Masks):
    """
    ...
    dvar_rho = GridShift.Vpt_to_Rho(dvar_v)*Masks['RhoMask']
...
def y_grad_v(RomsNC, var, dy, Masks):
    """
...
def z_grad(tstep, var, dz, Masks) :
    ...
    dvar_rho = GridShift.Wpt_to_Rho(dvar_w)*Masks['RhoMask']
